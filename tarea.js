...
function tienenMismaLongitud(str1, str2) {
    ...
    // Tu código:
    if(str1 === str2){
      console.log( true + " (Si Tienen la misma Longitud)");
    }else{
...
const nuevoUsuario = (nombre, email, password) => ({nombre, email, password});
    ...
    // Devuelve el objeto
    // Tu código:
  const usuario = nuevoUsuario ('Janer','janermora64@gmail.com',123);
  console.log(usuario);
  ...
  const verificarPassword = (usuario, password) => {
    ...
    // De lo contrario, devuelve "false"
    // // Tu código:
   if (usuario.password.value=='CONTRASEÑA' && usuario.nombre.value=='USUARIO'){ 
      return true;
  } 
  else{ 
  ...
  const actualizarPassword = (usuario, nuevaPassword) => {
    ...
    // Tu código:
    nuevaPassword = 444
    usuario[3].password = nuevaPassword
    console.log(usuario);
  }
...
const crearInstanciaPersona = (nombre, apellido, edad, domicilio)=>{
    ...
    //Recibirá los valores "Janer", "Mora", 18, "Aurora 123" para sus respectivas propiedades
    //Devolver la nueva persona creada

    var persona = new Persona ('Janer','Mora',18,'Aurora 123')
    console.log(persona);
}
...
const copiarEach = (array, cb) =>{
  ...
  function filter(array) {
    ...
    const letras = consulta.split(""); //divide las letras individualmente
    const filtrado = array.filter(mes => { //filtra los meses
      for(letra of letras){ //recorre las letras
        if(mes.indexOf(letra) !== -1) 
          return true;
      } 
      return false;
    });
